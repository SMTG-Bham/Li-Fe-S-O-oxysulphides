...
class PredictG(object):
    ...
    def __init__(self, 
        ...
        self.path_to_masses = path_to_masses if path_to_masses else str(base / 'masses.json')
        self.path_to_chempots = path_to_chempots if path_to_chempots else str(base / 'Gels.json')
...
def make_g_diagram(phased, T):
    """
    Make a diagram for gibbs formation energy using the SSIO descriptor package.
    
    The elemental species are assumed to have zero formation energy, and corrections are
    added to the compound's formation enthalpy. 

    Args:
        phased (PhaseDiagram): A phase diagram of formation enthalpy to be converted to Gibbs formation energy diagram
        T (int): Target temperature, must to multiple of hundreds
    
    Returns:
        A new phase diagram whose energies are Gibbs energy of formation at certain temperature
    """
    new_entries = []
    for entry in phased.all_entries:
        form_eng = phased.get_form_energy_per_atom(entry)
        pg = PredictG(entry.composition.formula, form_eng, None)  # Note that the input require per-atoms
        g = pg.dG(T, entry.parameters['volume'] / entry.composition.num_atoms)
        new_entry = ComputedEntry(entry.composition, g * entry.composition.num_atoms, parameters={'T': T, **entry.parameters})
        try:
            new_entry.attribute = entry.attribute
        except AttributeError:
            pass
        new_entries.append(new_entry)
    return PhaseDiagram(new_entries)
